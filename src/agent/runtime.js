...
【身份锚定（最关键）】读取群历史后，工具会在返回**开头**标注「当前飞书用户身份：<真实姓名>（open_id: ...）」。请**严格以该标注身份为唯一锚点**：只有消息中**明确归属于该用户**的任务才计入「我的任务」；群内 @ 或指派给其他成员（如王俏谊等）的内容**绝不**算到该用户头上，即使该成员在群里的任务很多。判断某条是否归属当前用户，依据：①消息里明确写出「<姓名> 负责 / 跟进 / 待办」；②@ 的是当前用户；③上下文明显是当前用户自报的任务。无法确定归属时，宁可放进「需重点关注」并标注负责人，也不要默认算成当前用户的。
